...
%(generator_code_pulseInput_xxx.m file) for desired value(s) of n.
...
testFilenameL=['yL_' num2str(nactual) '_' num2str(iactual) '_pulseInput.mat'];
yLload=load(testFilenameL);
yL=[yLload.yL];
testFilenameN=['yN_' num2str(nactual) '_' num2str(iactual) '_pulseInput.mat'];
...
txL=ifftshift(ifft(xL));
mtxL=real(txL);
txN=ifftshift(ifft(xN));
mtxN=real(txN);subplot(2,1,2);
p1=plot(t,mtxN,'--b','LineWidth',6);
hold on 
p2=plot(t,mtxL,':r','LineWidth',6);
hold on
loadFilename=['x_time_domain_pulseInput.mat'];
xload=load(loadFilename);
x=[xload.x];
p3=plot(t,x,'g+','LineWidth',2.5);
p1.Color(4) = 1.00;
p2.Color(4) = 1.00;
p3.Color(4)= 0.05;
title({
...
ax = gca;
ax.FontSize = 13;
legend({'mtxN(t) = Partial discharge signal predicted from short circuit neutral current','mtxL(t) = Partial discharge signal predicted from short circuit line current','x(t) = Actual input partial discharge signal'},'Location','northwest')
filename=['mtxN_vs_mtxL_actual_' num2str(nactual) '_' num2str(iactual) '_potential_' num2str(npotential) '_' num2str(ipotential) '_pulseInput'];
...
%currently the below two predictions seem to fail for some reason
percent_rmse_x_mtxN=mean(sqrt((mtxN-x').*conj(mtxN-x')))/mean(x')*100
percent_rmse_x_mtxL=mean(sqrt((x'-mtxL).*conj(x'-mtxL)))/mean(x')*100